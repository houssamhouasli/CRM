        ...
        <div class="card-body py-3"> 
            <div class="row g-2 align-items-center">
                ...
                <div class="col-md-5 text-md-end text-start mt-2 mt-md-0">
                    <a href="{{ route('commercial.clients.create') }}" class="btn btn-primary-custom btn-sm">
                    ...
                    <span wire:loading.remove style="font-size: 0.8rem; color: var(--text-secondary);" class="ms-2">
                    ...
                    <tbody> 
                        @forelse($clients as $client)
                            <tr>
                                ...
                                <td>{{ $client->email }}</td>
                                <td>{{ $client->phone ?? '—' }}</td>
                                <td>{{ number_format($client->deliveries_sum_total_ttc ?? 0, 2, ',', ' ') }} MAD</td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end gap-1">
                                        <a href="{{ route('commercial.clients.show', $client) }}" class="btn btn-outline-custom btn-sm" title="Voir"><i class="bi bi-eye"></i></a>
                                        <a href="{{ route('commercial.clients.edit', $client) }}" class="btn btn-outline-custom btn-sm" title="Modifier"><i class="bi bi-pencil"></i></a>
                                        <form action="{{ route('commercial.clients.destroy', $client) }}" method="POST" class="d-inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce client ?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-outline-custom btn-sm text-danger border-danger" title="Supprimer"><i class="bi bi-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="text-center text-muted py-5 text-center">
